...
class MediaPlayer extends QPCanvas {
  ...
  constructor(canvasHandle, media, layerCount = 10) {
    ...
    this.settings = new MediaSettings(
      this.canvas,
      this.media,
      IDTag.getGenericIDTag("MediaSettingsObj")
    );
    this.background = new QPBackground(
      "/assets/ThemeDefault_Build.png",
      this.canvas,
      "MediaPlayerBackground",
      "rgba(0, 0, 0, 0.5)"
    );
    this.settings.loadMenu();
    this.settings.menu.setExternalMenuFunction(
      SETTINGS_TOGGLE_DEBUG_MODE,
      this.toggleDebugMode.bind(this)
    );

    this.mediaName = new TextField(
      this.canvas,
      this.formatMediaName(this.media.getMediaTitle()),
      undefined,
      "white"
    );
    this.mediaTime = new TextField(
      this.canvas,
      undefined,
      "--:--/--:--",
      "white"
  ...
  setDefaultStage() {
    ...
    this.registerWidget(this.background, 1, () => {
      const backgroundY =
        this.calculateTimelineHeight(this.canvas.clientHeight) - 30;
    ...
    this.registerWidget(this.mediaTime, textLayer, () => {
      ...
      this.mediaTime.setLabel(
        this.formatMediaDuration(
  ...
  formatMediaDuration(currentTime, duration) {
    return `${QPUtility.secondsToTimeString(
      currentTime
    )}/${QPUtility.secondsToTimeString(duration)}`;
  }
  ...
  reportHoverEvents(mX, mY) {
    ...
    for (const item of QPDrawList) {
      ...
      if (isHovering) {
        ...
        console.debug(
          `[${item.name}](${item.x}, ${item.y}) is hovered by mouse @(${mX}, ${mY})`
        );
  ...
  onClick(event) {
    ...
    if (this.settings.hovered) {
      ...
      this.settings.menu.y =
        this.settings.y -
        this.settings.height * this.settings.settingsList.length;
